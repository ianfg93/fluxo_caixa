...
export default function EditNFePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoadingData(true)
      await Promise.all([loadVendors(), loadProducts(), loadInvoice()])
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNfeData({
          ...
          nfeSeries: invoice.nfeSeries,
          nfeAccessKey: invoice.nfeAccessKey || "",
          nfeProtocol: invoice.nfeProtocol || "",
        ...
        // Preencher valores
        setValues({
          totalProducts: invoice.totalProducts.toString(),
          totalTax: invoice.totalTax.toString(),
          freightValue: invoice.freightValue.toString(),
          insuranceValue: invoice.insuranceValue.toString(),
          discountValue: invoice.discountValue.toString(),
          otherExpenses: invoice.otherExpenses.toString(),
          icmsValue: invoice.icmsValue.toString(),
          ipiValue: invoice.ipiValue.toString(),
          pisValue: invoice.pisValue.toString(),
          cofinsValue: invoice.cofinsValue.toString(),
        })
        ...
        setPayment({
          ...
          installments: invoice.installments.toString(),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await ApiClient.put(`/api/nfe/${params.id}/edit`, {
        ...nfeData,
        ...values,
        ...payment,
        totalInvoice,
      ...
      if (response.ok) {
        const data = await response.json()
        alert("NF-e atualizada com sucesso!")
        router.push(`/nfe/${params.id}`)
      } else {
        const error = await response.json()
        alert(`Erro ao atualizar NF-e: ${error.error}`)
      }
    } catch (error) {
      console.error("Error updating NFe:", error)
      alert("Erro ao atualizar NF-e. Verifique se você está autenticado.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <AuthenticatedLayout>
      <div className="container mx-auto py-6 px-4 max-w-7xl">
        ...
        <div className="flex items-center gap-4 mb-6">
          <Button variant="outline" onClick={() => router.push(`/nfe/${params.id}`)}>
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Editar Nota Fiscal de Entrada</h1>
            <p className="text-muted-foreground mt-1">Modifique os dados da NF-e antes do processamento</p>
          </div>
        </div>

        {loadingInvoice ? (
          <div className="text-center py-12">
            <p className="text-muted-foreground">Carregando dados da NF-e...</p>
          </div>
        ) : (
          <>
        {/* Alerta informativo */}
        <Alert className="mb-6">
          ...
          <AlertDescription>
            Esta NF-e ainda não teve estoque ou contas a pagar processados. Você pode editar todos os dados. Após salvar, será necessário processar manualmente.
          </AlertDescription>
        </Alert>
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-4">
            <Button variant="outline" onClick={() => router.push(`/nfe/${params.id}`)} className="flex-1">
              Cancelar
            </Button>
            <Button onClick={handleSubmit} disabled={loading} className="flex-1">
              <Save className="mr-2 h-4 w-4" />
              {loading ? "Salvando..." : "Atualizar NF-e"}
            </Button>
          </div>
        </div>
        </>
